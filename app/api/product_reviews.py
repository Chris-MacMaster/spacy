...
def get_review(review_id):
    ...
    if review:
        ...
        if product_images:
            review_payload['Product']['ProductImages'] = [
                image.to_dict() for image in product_images]
        if shop:
...
def get_all_reviews():
    ...
    for review in payload.values():
        images = ReviewImage.query.filter(
            ReviewImage.review_id == review['id']).all()
...
def get_reviews_of_product(product_id):
    ...
    reviews = ProductReview.query.filter(
        ProductReview.product_id == product_id).all()
    payload = {}
    for review in reviews:
        payload[review.id] = review.to_dict()
    for review in payload.values():
        review_image = ReviewImage.query.filter(
            ReviewImage.review_id == review['id']).first()
        if review_image:
