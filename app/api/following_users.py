...
def get_followers_and_following():
    ...
    if current_user.is_authenticated:
        followers = FollowingUsers.query.filter(
            FollowingUsers.follower_id == current_user.id)
        following = FollowingUsers.query.filter(
            FollowingUsers.followed_id == current_user.id)
